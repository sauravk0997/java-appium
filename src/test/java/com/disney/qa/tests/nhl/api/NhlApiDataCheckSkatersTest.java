...
public class NhlApiDataCheckSkatersTest extends BaseNhlApiTest {
    /**
     * JIRA ticket: https://jira.bamtechmedia.com/browse/QAA-1360
     */
    @QTestCases(id = "42998")
    @MethodOwner(owner = "shashem")
    @Test(dataProvider = "DataProvider", description = "Verify: Team Page - Skaters")
    @XlsDataSourceParameters(sheet = "abbreviation", dsUid = "TUID, Name")
    public void verifyTeamsSkater(Map<String, String> data) throws IOException, ProcessingException {

        List<String> leaderNodes = new ArrayList<>();
        leaderNodes.add("points");
        leaderNodes.add("goals");
        leaderNodes.add("assists");
        leaderNodes.add("plusMinus");
        leaderNodes.add("powerPlayGoals");
        leaderNodes.add("shortHandedGoals");
        leaderNodes.add("timeOnIcePerGame");
        leaderNodes.add("faceOffPct");

        SoftAssert softAssert = new SoftAssert();

        for (String nodes : leaderNodes) {
