...
public class DisneyPlusHulkDetailsTest extends DisneyBaseTest {
    ...
    public void verifyJuniorProfileDetailsUnavailableState() {
        ...
        applePage.clickOpenButton();

        sa.assertTrue(applePage.getUnavailableContentError().isPresent(), "Unavailable content error not present.");
        sa.assertTrue(applePage.getUnavailableOkButton().isPresent(), "Unavailable content error button not present.");
