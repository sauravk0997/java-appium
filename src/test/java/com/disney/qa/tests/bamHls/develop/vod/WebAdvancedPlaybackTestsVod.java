...
public class WebAdvancedPlaybackTestsVod extends bamHlsBaseTest {
    ...
    @QTestCases(id = "619")
    @Test(description = "Player - Seek to future point and seek back to past point", priority = 3)
    public void testSeekFutureBackPoint() {
        SoftAssert sa = new SoftAssert();
        ...
        mP.seekOffsetInt(1000);

        sa.assertTrue(995 <= mP.getCurrentStreamTime() && mP.getCurrentStreamTime() <= 1005 ,
               mP.assertStreamRangeTrue("1000", mP.getCurrentStreamTime()));
        sa.assertEquals(mP.getDesiredMediaPlayerStateStatus(PLAYING), PLAYING,
                mP.assertMessageMediaPlayerStatus(PLAYING));

        mP.seekOffsetInt(500);

        sa.assertTrue(495 <= mP.getCurrentStreamTime() && mP.getCurrentStreamTime() <= 505,
                mP.assertStreamRangeTrue("500", mP.getCurrentStreamTime()));
        sa.assertEquals(mP.getDesiredMediaPlayerStateStatus(PLAYING), PLAYING,
                mP.assertMessageMediaPlayerStatus(PLAYING));
    ...
    @QTestCases(id = "620")
    @Test(description = "Player - Seek multiple times forward")
    public void testSeekForwardMultiple() {
        SoftAssert sa = new SoftAssert();
        ...
        mP.seekOffsetInt(200);

        sa.assertEquals(mP.getDesiredMediaPlayerStateStatus(PLAYING), PLAYING,
                mP.assertMessageMediaPlayerStatus(PLAYING));
        sa.assertTrue(195 <= mP.getCurrentStreamTime() && mP.getCurrentStreamTime() <= 205 ,
                mP.assertStreamRangeTrue("200", mP.getCurrentStreamTime()));

        mP.seekOffsetInt(400);

        sa.assertEquals(mP.getDesiredMediaPlayerStateStatus(PLAYING), PLAYING,
                mP.assertMessageMediaPlayerStatus(PLAYING));
        sa.assertTrue(395 <= mP.getCurrentStreamTime() && mP.getCurrentStreamTime() <= 405 ,
                mP.assertStreamRangeTrue("400", mP.getCurrentStreamTime()));

        mP.seekOffsetInt(600);

        sa.assertEquals(mP.getDesiredMediaPlayerStateStatus(PLAYING), PLAYING,
                mP.assertMessageMediaPlayerStatus(PLAYING));
        sa.assertTrue(595 <= mP.getCurrentStreamTime() && mP.getCurrentStreamTime() <= 605 ,
                mP.assertStreamRangeTrue("600", mP.getCurrentStreamTime()));

        mP.seekOffsetInt(800);

        sa.assertEquals(mP.getDesiredMediaPlayerStateStatus(PLAYING), PLAYING,
                mP.assertMessageMediaPlayerStatus(PLAYING));
        sa.assertTrue(795 <= mP.getCurrentStreamTime() && mP.getCurrentStreamTime() <= 805 ,
                mP.assertStreamRangeTrue("800", mP.getCurrentStreamTime()));
    ...
    @QTestCases(id = "621")
    @Test(description = "Player - Seek multiple times backward", priority = 5)
    ...
    @QTestCases(id = "622")
    @Test(description = "Player - Seek multiple times away from completed tail", priority = 6)
    public void testSeekAwayTail() {
        SoftAssert sa = new SoftAssert();

        mP.playUrlByStreamEndVod(BAM_HLS_WEB_QA_TEST_STREAMS, WWE_TS_VOD, 1736, 65);
    ...
    @QTestCases(id = "631")
    @Test(description = "Player - Short chunks do not impact playback", priority = 7)
    ...
    @QTestCases(id = "671")
    @Test(description = "Player - Playback rate can be adjusted without crashing player", priority = 8)
    public void testPlaybackRateAdjust() {
        SoftAssert sa = new SoftAssert();

        ArrayList<String> assertArray = new ArrayList<>(Arrays.asList("0.25", "4", "0.5", "1", "2"));
        ...
        mP.waitForVideoPause(1, true);

        dataArray.add(mP.getPlaybackRate());

        mP.playSetPlaybackRate(0.5);
        mP.waitForVideoPause(10, true);
    ...
    @QTestCases(id = "672")
    @Test(description = "Player - Playback rates greater than 1x revert to 1x when end of buffer is reached", priority = 9, enabled = false)
